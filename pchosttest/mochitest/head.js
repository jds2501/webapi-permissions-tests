...
function ok(expression, message) {
	console.log('TEST RESULT = ' + expression + ', MESSAGE = ' + message);
}

function is(first, second, message) {
...
function unexpectedEventAndFinish(message, eventName) {
	console.log('TEST FAILED WITH UNEXPECTED EVENT for ' + eventName);
	console.log(message);
	finish();
}
